...
export function isValidTimeString(timeString: string): boolean {
  ...
  if (parts.length === 2) {
    ...
    if (!/^\d+$/.test(minutesStr) || !/^\d{1,2}$/.test(secondsStr))
      return false;
...
export function formatSecondsToTimeString(totalSeconds: number): string {
  const safeSecs = totalSeconds < 0 ? 0 : totalSeconds;
...
export function formatMinutesToTimeString(minutes: number): string {
  const safeMins = minutes < 0 ? 0 : minutes;

  const mins = Math.floor(safeMins);
