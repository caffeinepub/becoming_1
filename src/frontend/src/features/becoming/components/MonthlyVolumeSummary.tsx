...
export function MonthlyVolumeSummary({ habits, selectedMonth }: MonthlyVolumeSummaryProps) {
  ...
  habits.forEach((habit) => {
    ...
    if (raw.type === 'reps') {
      ...
      totalNumeric += raw.value;
    }
  });

  const hasReps = habits.some((h) => h.volumeTracking?.unitType === 'reps');
  const hasTime = habits.some((h) => h.volumeTracking?.unitType === 'time');
  const hasNumeric = habits.some((h) => {
    const unitType = h.volumeTracking?.unitType;
    return unitType && unitType !== 'reps' && unitType !== 'time';
  });

  // Don't render if no habits have volume tracking
  if (!hasReps && !hasTime && !hasNumeric) {
    return null;
  }
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        {hasNumeric && (
          <div className="flex flex-col">
            ...
            <span className="text-lg font-bold text-primary">{totalNumeric}</span>
          </div>
        )}
      </div>
    </div>
