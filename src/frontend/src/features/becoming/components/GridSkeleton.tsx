    ...
    <div className="space-y-6">
      ...
      <div className="bg-card border border-border rounded-lg overflow-hidden">
        ...
        <div className="flex gap-2 p-2 border-b border-border overflow-x-auto">
          {Array.from({ length: 12 }, (_, i) => `tab-${i}`).map((key) => (
            <Skeleton key={key} className="h-10 w-16 flex-shrink-0" />
          ))}
        </div>
        ...
        <div className="overflow-x-auto">
          <div className="inline-block min-w-full">
            ...
            <div className="flex border-b-2 border-border bg-muted/50">
              <div
                className={`${HABIT_INFO_WIDTH} flex-shrink-0 px-4 py-3 sticky left-0 z-10 bg-muted/50 md:static md:z-auto`}
              >
            ...
            <div className="divide-y divide-border">
              {Array.from({ length: 3 }, (_, i) => `row-${i}`).map((key) => (
                <div key={key} className="flex">
                  <div
                    className={`${HABIT_INFO_WIDTH} flex-shrink-0 px-3 py-3 sticky left-0 z-10 bg-background md:static md:z-auto`}
                  >
